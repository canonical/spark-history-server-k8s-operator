...
class SparkHistoryServerConfig(WithLogging):
    ...
    def __init__(
            self,
            s3_connection_info: Optional[S3ConnectionInfo], ingress_url: Optional[str]
    ):
    ...
    def _s3_conf(self) -> dict[str, str]:
        return {
            "spark.hadoop.fs.s3a.endpoint":
                self.s3_connection_info.endpoint or "https://s3.amazonaws.com",
        ...
        } if self.s3_connection_info else {}
    ...
    def contents(self) -> str:
        ...
        return "\n".join([
            f"{key}={value}"
            for key in sorted(dict_content.keys()) if (value := dict_content[key])]
        )
