# Enable and configure monitoring for Charmed Apache Spark History Server

Charmed Spark History Server come with the [JMX exporter](https://github.com/prometheus/jmx_exporter/).
The metrics can be queried by accessing the `http://<history-server-unit-ip>:9101/metrics endpoint.

Additionally, Charmed Apache Spark History Server supports native integration with the Canonical Observability Stack (COS).

This section is about monitoring the Apache Spark History Server, for more information about the underlying Spark jobs and their COS integration, refer to the [COS documentation](https://charmhub.io/topics/canonical-observability-stack) and the [monitoring explanation section](/t/charmed-spark-documentation-explanation-monitoring/14299).
...
Switch to COS K8s environment and offer COS interfaces to be cross-model integrated with Charmed Spark History Server K8s model:
...
### Consume offers via the Apache Spark History Server model

Switch back to the Charmed Apache Spark History Server K8s model, find offers and integrate with them:
...
Finally, integrate `grafana-agent` it with Charmed Apache Spark History Server K8s:
...
Then, browse to the **Dashboards** section, where you can find a **Spark History Server JMX Dashboard** one.
