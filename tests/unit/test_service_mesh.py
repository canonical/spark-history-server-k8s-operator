...
def test_service_mesh_relation_adds_labels(
    ...
    """Ensure that joining the service-mesh relation applies the beacon's ambient labels to History Server."""
    ...
    history_server_ctx.run(history_server_ctx.on.relation_changed(mesh_relation), state)
...
def test_service_mesh_relation_broken_removes_labels(
    ...
    """Ensure that breaking the service-mesh relation clears the ambient labels from History Server."""
    mesh_relation = Relation(
        endpoint=SERVICE_MESH_RELATION,
        interface="service_mesh",
        remote_app_name="istio-beacon-k8s",
    ...
    history_server_ctx.run(history_server_ctx.on.relation_broken(mesh_relation), state)
